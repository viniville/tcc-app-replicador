...
public class SamplesUtility {
    ...
    public static void crateIfNotExists(File dir) {
        if (!dir.exists() || !dir.isDirectory()) {
            if (!dir.mkdir()) {
                throw new RuntimeException("Erro ao criar diretório " + dir.getName());
            }
        }
    }

    public static void crateIfNotExists(String dir) {
        crateIfNotExists(new File(dir));
    }
}